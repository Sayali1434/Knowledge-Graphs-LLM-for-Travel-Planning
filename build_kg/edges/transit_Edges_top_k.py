...
def build_pair_edges(left_stop_rows, right_stop_rows, left_key, right_key):
    ...
    for edge in candidate_edge_list:
        ...
        current_count = edges_per_left.get(left_id, 0)

        if current_count < TOP_K:
            top_edges.append(edge)
            edges_per_left[left_id] = current_count + 1

    print(
        f"{left_key}-{right_key} candidate pairs: {len(candidate_edge_list)}"
    )
    print(
        f"{left_key}-{right_key} top pairs selected: {len(top_edges)} "
        f"across {len(edges_per_left)} {left_key} nodes"
    )
